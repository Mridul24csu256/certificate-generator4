...
# ------------------ Helper Functions ------------------
def get_font_style(font_file, size, bold=False, italic=False):
    """Return a PIL ImageFont from uploaded font or default if none."""
    if font_file:
...
def snap_position(x, y, text_width, text_height, canvas_width, canvas_height, other_boxes=[]):
    """Snap text to center or nearby boxes."""
...
for col in selected_columns:
    ...
    for i in range(num_boxes):
        ...
        font = get_font_style(font_file, font_size, bold, italic)
...
if st.button("👀 Preview First Certificate"):
    ...
    for col, boxes in column_text_boxes.items():
        for box in boxes:
            ...
            font = get_font_style(font_file, box["size"], box["bold"], box["italic"])

            bbox = draw.textbbox((0, 0), text, font=font)
            x, y = box["pos"]
            if box["bold"] and font_file:
